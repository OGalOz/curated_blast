...
my $tmpDir = "../tmp";
#We clear out the temporary directory
system("python3 ../bin/clear_dir.py $tmpDir");
my $procId = $$;
...
unless($isNuc) {
  ...
  system("$usearch -ublast $chitsfaaFile -db $seqFile -evalue $maxEvalue -blast6out $ublastFile > ../tmp/usearch_mmseqs_out") == 0
    || die "usearch failed: $!";
  unlink($seqFile);
...
if (! -e $xfile) {
  system("$usearch -fastx_findorfs $fnafile -aaout $xfile -orfstyle 7 -mincodons $minCodons > ../tmp/fastx_replace_out") == 0
    || die "usearch findorfs failed: $xfile $!";
}
...
system("$usearch -ublast $chitsfaaFile -db $xfile -evalue $maxEvalue -blast6out $ublastFile > ../tmp/ublast_second_log") == 0
  || die "usearch failed: $!";
